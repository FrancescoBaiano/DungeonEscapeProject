...
void ADungeonEscapeCharacter::Interact()
{
	...
	DrawDebugSphere(GetWorld(), end, interactionSphereRadius, 12, FColor::Green, false, 5.f);
	...
	if(hasHit)
	{
		...
		if (hitActor && hitActor->ActorHasTag("CollectableItem"))
		{
		...
		else if (hitActor && hitActor->ActorHasTag("Lock"))
		{
			ALock* lockActor = Cast<ALock>(hitActor);
			if (lockActor)
			{
				if (!lockActor->GetKeyPlaced())
				{
					int32 itemRemoved = itemList.RemoveSingle(lockActor->keyItemName);
					if(itemRemoved)
					{
						lockActor->SetKeyPlaced(true);
					}
					else 
					{
						UE_LOG(LogTemp, Warning, TEXT("KeyItem not in inventory"));
					} 
				}
				else 
				{
					itemList.Add(lockActor->keyItemName);
					lockActor->SetKeyPlaced(false);
				}
			}
		}
		else
